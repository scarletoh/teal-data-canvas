...
const Navbar = () => {
  ...
  return (
    <nav className={cn(
      ...
      scrolled ? "bg-darkBlue/95 shadow-lg backdrop-blur-sm" : "bg-transparent"
    )}>
      ...
      {/* Mobile menu */}
      {isMenuOpen && (
        <div className="md:hidden bg-darkBlue/95 backdrop-blur-sm">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="block px-3 py-2 text-base font-medium text-gray-300 hover:text-teal hover:bg-secondary/50 rounded-md"
